functions{
  ...
  real calc_tpm(int s_from, int s_to, vector intercept, real d, real lambda,real t,real gamma, real senti, real epsilon ,array[] vector l_tpm){
    real diag;
  ...
  row_vector compute_ll(array[] int Rating, array[] real Days, array[] real Sentiment, int T , int S , vector l_pi , array[] vector l_tpm , array[] vector emission, vector intercept, real lambda, real gamma, real epsilon){
  ...
  real partial_sum(array[] int slice_restaurants,
                   int start, int end,
                   // Data arrays
                   array[] int Time,
                   array[] int Ratings,
                   array[] real Days,
                   array[] real Sentiment,
                   array[] int pos_obs,
                   // Model dimensions
                   int S,
                   // Parameters
                   vector l_pi,
                   array[] vector l_tpm,
                   array[] vector emission,
                   vector intercept,
                   real lambda,
                   real gamma,
                   real epsilon) {
    real partial_log_lik = 0.0;

    for(i in 1:num_elements(slice_restaurants)){
      int m = slice_restaurants[i];
      array[Time[m]] int Rating_vec = Ratings[pos_obs[m]:(pos_obs[m] + Time[m] - 1)];
      array[Time[m]] real Days_vec = Days[pos_obs[m]:(pos_obs[m] + Time[m] - 1)];
      array[Time[m]] real Sentiment_vec = Sentiment[pos_obs[m]:(pos_obs[m] + Time[m] - 1)];
      row_vector[S+1] results;

      results = compute_ll(Rating_vec, Days_vec, Sentiment_vec, Time[m],
                           S, l_pi, l_tpm, emission, intercept, lambda, gamma, epsilon);
      partial_log_lik += results[S+1];
    }
...
data {
  ...
  int<lower=1> grainsize; // grainsize for reduce_sum (default: 1)
}
transformed data {
  //prior parameter vectors for states and pain levels
  array[N_total] int pos_obs;
  array[S] vector[S-1] prior_tpm;
  array[N_train] int train_indices = linspaced_int_array(N_train, 1, N_train);
  {
    for(s in 1:(S)){
...
model {
  ...
  // Parallelized likelihood computation using reduce_sum
  target += reduce_sum(partial_sum,
                       train_indices,
                       grainsize,
                       Time, Ratings, Days, Sentiment, pos_obs,
                       S,
                       l_pi, l_tpm, emission, intercept,
